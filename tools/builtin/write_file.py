from tools.base import Tool, ToolKind, ToolInvocation, ToolResult , FileDiff , ToolConfirmation
...
class WriteFileParams(BaseModel):
    path: str = Field(
        ...,
        description= 'Path to the file to write (relative to working dir or absolute path)'
    )
    content: str = Field(
        ...,
    ...
    async def execute(self, invocation: ToolInvocation) -> ToolResult:
        ...
        try:
            ...
            return ToolResult.success_result(
                f"{action} {line_count} lines to {path}",
                diff=FileDiff(
                    path=path,
                    old_content=old_content,
                    new_content=params.content,
