...
from tools.base import Tool, ToolResult, FileDiff, ToolKind
from utils.paths import resolve_path, ensure_parent_directories
from tools.base import ToolInvocation
from pathlib import Path
class EditParams(BaseModel):
    path: str = Field(..., description="Path to the file to edit (relative to working directory or absolute path)",)
    old_string: str = Field(..., description="The exact text to find and replace. Must match exactly including all whitespace and indentation. For new files, leave this empty.",)
    new_string: str = Field(..., description="The text to replace old_string with. Can be empty to delete text",)
...
class EditFileTool(Tool):
    ...
    async def execute(self, invocation: ToolInvocation):
        ...
        try:
            ...
            return ToolResult.error_result(
                f"Failed to write to {path}: {str(e)}"
            )
