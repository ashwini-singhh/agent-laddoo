...
class TUI:
    def __init__(self, config: Config, console: Console | None = None) -> None:
        ...
        self.assistant_streaming_open = False
        self.cwd = config.cwd
        self._tools_args_by_call_id: dict[str , dict[str , Any]] = {}
        self._max_block_tokens = 2000 # Default limit
    ...
    def begin_assistant(self) -> None:
        ...
        self.assistant_streaming_open = True

    def end_assistant(self) -> None:
        if self.assistant_streaming_open:
            self.console.print()
        self.assistant_streaming_open = False
    ...
    def display_tool_call(self, message: str) -> None:
        self.console.print(f"[tool]🛠️  {message}[/tool]")

    def display_tool_result(self, message: str) -> None:
        self.console.print(f"[tool]✅ {message}[/tool]")

    def display_tool_error(self, message: str) -> None:
    ...
    def tool_call_start(
        ...
        self._tools_args_by_call_id[call_id] = arguments
    ...
    def _extract_read_file_code(self, text: str) -> tuple[int, str] | None:
        body = text
        # Be permissive with the header regex
        header_match = re.search(r"Showing lines (\d+) - (\d+) of (\d+)\n\n", text)
        ...
        for line in lines:
            # Match line number followed by a pipe and then the content
    ...
    def tool_call_complete(
        ...
        args = self._tools_args_by_call_id.get(call_id, {})
        ...
        if name == "read_file" and success:
            shown_start = metadata.get("show_start")
            shown_end = metadata.get("show_end")
            ...
            if primary_path:
                ...
                blocks.append(Text("".join(header_parts), style="highlight"))
...
def truncate_text(text: str, model_name: str, max_tokens: int) -> str:
    # Use a simple char-based fallback for now to avoid the missing truncate_text error
